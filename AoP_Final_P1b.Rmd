...
sw_ols_clean <- sw_ols_clean %>%
  dplyr::filter(Opinion_seaweed != "4") #final n is 592

ma_ols_clean <- ma_ols_all %>%
  dplyr::filter(Opinion_aquaculture != "7")
ma_ols_clean <- ma_ols_clean %>%
  dplyr::filter(Opinion_aquaculture != "4") #final n is 690
...
sw_age_group_contrasts <- dunnTest(Opinion_seaweed ~ Age_factor, data = sw_group_all, method = "bh") #weak association 40 to 49 - 70+
...
#Uniformity, dispersion, and outliers
plotQQunif(output3) #Dispersion: p = 0.83, Outlier: p = 0.59, KS test: p = 0.96, deviations not significant.
plotResiduals(output3) #ok with large sample size
#query the model
Anova(glm3)

#Interpret as odds-ratios with 95% CI
ci_sw_M3 <- confint(glm3)
OR_sw <- exp(cbind(OR = coef(glm3), ci_sw_M3))
#pull pairs/contrasts for contrast table
...
```{r, Data for Figure 1a-b, echo=FALSE}
...
freq_3 <- c(76, 60, 60, 60, 45, 44, 35, 23)
...
s2_heat_map_group <- ggplot(s2_graph_data, aes(y = message_factor, x = Opinion_aqua_pre, fill = color, alpha = rel.freq)) +
  ...
  coord_cartesian(expand = FALSE) +
   theme(axis.text.x = element_text(angle=-56, hjust=0, size = 6),
        axis.text.y = element_text(size=6),
...
ggsave("figure_2a.png", plot = figure_2a, width = 3.5, height = 3.3, units = "in", dpi = 600)
...
figure_2b <- s3_heat_map_group +
  ...
  theme(legend.position = "none") +
  theme(panel.spacing = unit(1,"pt"))+
  ggtitle("2b: SW survey messages") +
  theme(plot.title = element_text(size = 8, hjust = -0.28, face = "bold"))

ggsave("figure_2b.png", plot = figure_2b, width = 3.5, height = 3.3, units = "in", dpi = 600)
...
ggsave("figure_2.tiff", plot = figure_2, width = 7, height = 4, units = "in", dpi = 600)
